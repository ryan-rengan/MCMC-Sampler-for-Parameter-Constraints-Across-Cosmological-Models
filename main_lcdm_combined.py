...
Runs the LCDM MCMC twice using the two H0 priors:
    1) H0 = 68.0, sigmaH0 = 2.8
    2) H0 = 73.8, sigmaH0 = 2.4

Then plots both LCDM confidence contour sets on the same PNG.
...
# -----------------------------
# LCDM model (Theoretical)
# -----------------------------
def E_lcdm(z, omega_m, omega_lam):
    """Return the LCDM expansion function E(z)."""
    omega_k = 1.0 - omega_m - omega_lam                             # Solving for Omega_k based on Omega_m and Omega_Lam
    E2 = omega_m * (1 + z)**3 + omega_k * (1 + z)**2 + omega_lam    # Expansion Function (squared)
...
# Statistics
# -----------------------------
def chi2_eff_lcdm(theta, H0, sigmaH0):
    """
    ...
    """
    omega_m, omega_lam = theta
    E = E_lcdm(z, omega_m, omega_lam)                               # Expansion Function
...
def log_prior_lcdm(theta):
    ...
    if 0.0 < omega_m < 0.55 and 0.0 < omega_lam < 1.35:             # If the parameters are within range, return a log prior of 0.0 (equal probability for all allowed values)
        return 0.0

    return -np.inf                                                  # If the parameters are outside range, return negative infinity (zero probability for those values)


def log_probability_lcdm(theta, H0, sigmaH0):
    ...
    chi2 = chi2_eff_lcdm(theta, H0, sigmaH0)
...
def run_lcdm_case(
    ...
    ##Run one LCDM MCMC case and return samples + summary info
    ...
    ndim = 2                                                        # Number of dimensions of parameter space. Here two bc OmegaM & OmegaL
    ...
    sampler = emcee.EnsembleSampler(
        ...
        log_probability_lcdm,                                       # Evaluates each proposed position
    ...
    trace_filename = f"plots/lcdm_trace_H0_{H0_filename}.png"       
    ...
    min_chi2 = chi2_eff_lcdm(best_params, H0, sigmaH0)              # Calculates minimum chi-square value for the best-fit parameters  
...
def main():
    ...
    current_H0_case = run_lcdm_case(
        ...
        initial_guess=np.array([0.29, 0.72]),                       # Starting guess for cosmology parameters
        seed=12345,                                                 # Keep same "random". Changing seed gives different random run.
    )

    # H0 = 73.8, sigmaH0 = 2.4 case
    prior_H0_case = run_lcdm_case(
        case_name=r"$H_0 = 73.8 \pm 2.4$",                          # Defines case name
        H0=73.8,                                                    # Value for prior Hubble Constant
        sigmaH0=2.4,                                                # Value for Uncertainty in prior Hubble Constant
        initial_guess=np.array([0.32, 0.91]),                       # Starting guess for cosmology parameters (different from the first case, values taken from Farooq et al. 2013)
    ...
    g.plot_2d(
        ...
        legend_labels=[                                              # Labels for the legend. The r before the string indicates raw string, useful for LaTeX.
            "H_0 = 68.0 ± 2.8",
            "H_0 = 73.8 ± 2.4",
        ],
    )
    ...
    plt.ylabel(r"$\Omega_{\Lambda}$")                               # Label for y-axis
    plt.xlim(0, 0.55)                                               # Domain for x-axis (Omega_m)
    plt.ylim(0, 1.35)                                               # Range for y-axis (Omega_Lambda)

    # Median Point for Current H0 Case (H0 = 68.0)
    plt.scatter(
        current_H0_case["best_params"][0],                        # Omega_m best value for H0=68.0 case 
        current_H0_case["best_params"][1],                        # Omega_Lambda best value for H0=68.0 case
        color="blue",
        s=50,  
        marker="o",                                                 # Circle
    ...
    # Flat Universe Line (Omega_m + Omega_Lambda = 1)
    plt.plot([0, 1], [1, 0], color="gray", ls=":", lw=0.9, label=r"Flat Universe")                          # Plots the line Omega_m + Omega_Lambda = 1 (flat universe line)
    # Accelerating Universe Line (Omega_Lambda = Omega_m / 2)
    plt.plot([0, 1], [0, 0.5], color="gray", ls="--", lw=0.9, label=r"Accelerating/Decelerating Universe")  # Plots the line Omega_Lambda = Omega_m / 2 (accelerating/decelerating universe line)


    plt.legend(                                                     # Display legend on the plot.
        loc="upper left",                                           # Legend location
        fontsize=7.5                                                # Font size for the legend text
    ) 
    output_path = "plots/lcdm_combined_contours.png"                # Output path for the saved plot
...
if __name__ == "__main__":                                          # Tells python to actually call main() when this script is ran
    main()
